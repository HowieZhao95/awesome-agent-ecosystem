...
GitHub 搜索、限流或服务故障会使同步任务失败，避免提交不完整的核心数据。引用仓库返回 404/410 时保留原条目并在 Actions 日志中列出 warning，供人工修复链接；第三方目录站暂时不可用时也会显示 warning，本轮跳过该来源。无新增、无星标或仓库元数据变化时，不创建新 PR。质量门禁运行测试、数据校验，并检查生成文件与 YAML 一致。
